#!/usr/bin/env bash
# Run a command inside the IIC-OSIC-TOOLS container with the repo mounted
# at /work. The container already defaults to PDK=ihp-sg13g2.
#
#   ./run.sh librelane config.json -v
...
exec docker run --rm \
    -v "$(git rev-parse --show-toplevel):/work" -w /work/digital \
    --user "$(id -u):$(id -g)" \
    -e PDK=ihp-sg13g2 \
